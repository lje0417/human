...
<body>
    <div class="container">
        ...
        <div class="search_container">
            <div class="search">
                &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
             <form id="searchform" action="st" method="get">
                <select id="select" name="optionvalue" class="inputTextHeight">
                    <option value="-----"
						...
						<option value="st_number"
							<c:if test="${map.optionvalue == 'sp_number'}">selected</c:if>>입고 NO</option>
						<option value="pm_name"
							<c:if test="${map.optionvalue == 'pm_name'}">selected</c:if>>품목명</option>
                </select>
            ...
            <table class="table">
                ...
                <tbody class="tbody">
                	<c:forEach var="SP" items="${map.list }">
                    	<tr>
                        	<td><input type="checkbox" class="delete-checkbox" value="${SP.stocking_number}"></td>
                        	<td>${SP.stocking_number}</td>
                        	<td><a href="/peanutbutter/modify_st?stocking_number=${SP.stocking_number}&part_code=${SP.part_code}">${SP.stocking_date}</a></td>
                        	<td>${SP.part_modelname}</td>
                        	<td>${SP.stocking_partner}</td>
                        	<td>${SP.stocking_currency}</td>
                        	<td>${SP.stocking_amount}</td>
                        	<td>${SP.stocking_warehouse}</td>
                        	<td>
                            <select id = select_status name="optionvalue_status" onchange="take(this, '${SP.stocking_number}')">
                                <option value="-----"
                                	<c:if test="${empty map.optionvalue || map.optionvalue == '-----'}">selected</c:if>>-------</option>
                                <option value="stay"
                                	<c:if test="${SP.stocking_status == 'stay'}">selected</c:if>>입고대기</option>
                                <option value ="underway"
                                	<c:if test="${SP.stocking_status == 'underway'}">selected</c:if>>입고 진행중</option>
                                <option value ="complete"
                                	<c:if test="${SP.stocking_status == 'complete'}">selected</c:if>>입고 완료</option>
                            </select>
                        </td>
                    </tr>
                   </c:forEach>
...
<%
		...
		<div style="text-align: center;">
			...
			<div class="regBtn_div">
				...
				<span class="Btn_css" id="regBtn" onclick="commitReg()">입고 입력</span>
			</div>
		</div>
		<form id="deleteone" action="delete_st" method="post">
			<input type="hidden" name="stocking_numbers" id="stocking_numbers">
		</form>
</body>
...
function searchTable() { // 돋보기 버튼을 클릭했을 때 동작하는 함수
	...
	let codesField = document.getElementById('stocking_numbers');
	codesField.value = selects.join(',');
	
    console.log('Stocking Numbers:', codesField.value);
	
	document.getElementById('deleteone').submit();
}
	function commitReg(){
    	location.href="/peanutbutter/insert_st";
    }	
    ...
    function searchpage(num){
        ...
        let url = "/peanutbutter/st?page=" + num;
...
//         	location.href="/peanutbutter/sp?optionvalue="+a+"&search="+b+"&page=num";
    ...
    function take(selectElement, stocking_number) {
    ...
    const selectedValue = selectElement.value;
        ...
        httpRequest.open("put", "/peanutbutter/update_st?stocking_status="+selectedValue+"&stocking_number="+stocking_number, true);
