				...
				<tbody class="tbody">
					<c:forEach var="Pl" items="${map.list }">
						<tr>
							...
							<td><a
								href="/peanutbutter/modify_pl?plan_number=${Pl.plan_number}&product_code=${Pl.product_code}">${Pl.plan_number}</a></td>
...
function searchTable() { // 돋보기 버튼을 클릭했을 때 동작하는 함수
    ...
    function commitReg(){
    	location.href="/peanutbutter/insert_pl";
    }
    	a = '${map.optionvalue}';
    	b = '${map.search}';
    ...
    function searchpage(num){
        ...
        let url = "/peanutbutter/pl?page=" + num;
...
//         	location.href="/peanutbutter/pt?optionvalue="+a+"&search="+b+"&page=num";
    ...
    function openPopup(planNumber) {
    	 window.open(
                 '/peanutbutter/remarks?plan_number=' + planNumber,
                 'popupWindow',
