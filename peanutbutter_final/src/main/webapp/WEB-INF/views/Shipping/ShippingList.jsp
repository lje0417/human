...
<body>
    <div class="container">
        ...
        <div class="search_container">
            <div class="search">
                &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
             <form id="searchform" action="sh" method="get">
                ...
                <tbody class="tbody">
                	<c:forEach var="SP" items="${map.list }">
                    	<tr>
                        	...
                        	<td><a
								href="/peanutbutter/modify_sh?shipping_number=${SP.shipping_number}&product_code=${SP.product_code}">${SP.shipping_date}</a></td>
                        	<td>${SP.product_name}</td>
                        	<td>${SP.shipping_arrival}</td>
                        	<td>${SP.shipping_currency}</td>
                        	<td>${SP.shipping_amount}</td>
                            <td>${SP.shipping_warehouse}</td>
                            <td>
                            <select id = select_status name="optionvalue_status" onchange="take(this, '${SP.shipping_number}')">
                                <option value="-----"
                                	<c:if test="${empty map.optionvalue || map.optionvalue == '-----'}">selected</c:if>>-------</option>
                                <option value="stay"
                                	<c:if test="${SP.shipping_status == 'stay'}">selected</c:if>>출고대기</option>
                                <option value ="underway"
                                	<c:if test="${SP.shipping_status == 'underway'}">selected</c:if>>출고 진행중</option>
                                <option value ="complete"
                                	<c:if test="${SP.shipping_status == 'complete'}">selected</c:if>>출고 완료</option>
                            </select>
                        </td>
                    </tr>
                   </c:forEach>
...
<%
		...
		<div style="text-align: center;">
			...
			<div class="regBtn_div">
				...
				<span class="Btn_css" id="regBtn" onclick="commitReg()">출고입력</span>
			</div>
		</div>
...
function searchTable() { // 돋보기 버튼을 클릭했을 때 동작하는 함수
	...
	function commitReg(){
    	location.href="/peanutbutter/insert_sh";
    }
...
//         	location.href="/peanutbutter/sh?optionvalue="+a+"&search="+b+"&page=num";
    ...
    function take(selectElement, shipping_number) {
        ...
        const selectedValue = selectElement.value;
            ...
            httpRequest.open("put", "/peanutbutter/update_sh?shipping_status="+selectedValue+"&shipping_number="+shipping_number, true);
