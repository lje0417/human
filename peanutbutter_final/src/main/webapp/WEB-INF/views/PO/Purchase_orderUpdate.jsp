    ...
    <div class="container">
        <div class="top">
            <div class="title">
                ...
                <span>${dto.purchase_order_number}</span>
            </div>
        </div>
        <div class="search_container">
			<form method="post" action="modify_po">
            <div class="searchcon">
            ...
            <div class="searchcon">
                ...
                <div class="search-box">
                    <select style="height: 40px;" class="제품select" name="purchase_order_currency" value="${dto.purchase_order_currency}">
                    <option value="-----">-----</option>                    
                    <option value="KRW"
                                	<c:if test="${dto.purchase_order_currency == 'KRW'}">selected</c:if>>대한민국 원(KRW)</option>
                    <option value="USD"
                                	<c:if test="${dto.purchase_order_currency == 'USD'}">selected</c:if>>미국달러(USD)</option>
                    <option value="JPY"
                                	<c:if test="${dto.purchase_order_currency == 'JPY'}">selected</c:if>>일본 엔(JPY)</option>
                    <option value="EUR"
                                	<c:if test="${dto.purchase_order_currency == 'EUR'}">selected</c:if>>유럽연합 유로(EUR)</option>
                    <option value="CNY"
                                	<c:if test="${dto.purchase_order_currency == 'CNY'}">selected</c:if>>중국 위안(CNY)</option>
                </select>
                </div>
               <div style="padding-right:15px;">출고 NO</div>
                <div class="search-box">
                    <input id="부품코드" type="text" name="purchase_order_number" value="${dto.purchase_order_number}" readonly>
                </div>
            </div>
            <div class="searchcon">
                ...
                <div class="search-box">
                    <input type="text" id="count" name="purchase_order_amount" value="${dto.purchase_order_amount}">
                </div>
                발주 일자
                <input style="height: 30px; font-size: 20px; margin-left: 11px;" type="date" id="date" max="2025-06-20"
                    min="2001-06-05" value="${dto.purchase_order_date}" name="purchase_order_date">
            </div>
            <div class="searchcon">
                <div style="padding-right:17px;">거래처</div>
                <div class="search-box">
                <input type="text" name="purchase_order_partner" value="${dto.purchase_order_partner}">
                </div>
            입고 예정일
                <input style="height: 30px; font-size: 20px; margin-left: 11px;" type="date" id="date" max="2025-06-20"
                    min="2001-06-05" value="${dto.warehousing_duedate}" name="warehousing_duedate">
            </div>
            <div class="searchcon">
                <div style="padding-right:17px;">입고예정 창고</div>
                <div class="search-box">
                <input type="text" name="purchase_order_warehouse" value="${dto.purchase_order_warehouse}">
                </div>
            </div>
...
<script>
    ...
    const inputCode = document.getElementById('부품코드');
